...
* 事件结构是用于等待事件发生，并执行相应条件分支，处理该事件。事件结构包括一个或多个子程序框图或事件分支，结构处理事件时，仅有一个子程序框图或分支在执行

* 程序框图 ---> 编程 ---> 结构 ---> 事件结构
* 事件结构等组成
  * 左上角是“事件超时”，默认为“-1”，默认永不超时。可以进行更改，单位ms
  * 上边框：是事件选择器
  * 左侧：事件数据节点
* 添加事件分支
  * 右键控件 ---> 添加分支
  * 默认只有一个“超时”分支
  * 当针对某一个控件添加了分支之后，无需将那个控件拖入或连接到事件结构
* 重排事件分支
  * 右键上边框的事件选择器 ---> 重排分支

## 45. 局部变量和全局变量（上）

* 局部变量可以从一个VI的不同位置访问前面板对象，全局变量可以在多个VI之间访问和传递数据
* 局部变量
  * 应用1：上次run的结果不会影响到下次run（比如bool类型的值，上次为T，下一次运行默为T运行）
  * 使用平铺式顺序结构，把现在的结构放在第一帧，再创建一个第0帧
  * 创建局部变量：右键输入输出控件 ---> 创建 ---> 局部变量（放在第0帧）
  * 在第0帧创建常量连接到创建的局部变量上，一个输入输出控件对应一个局部变量
