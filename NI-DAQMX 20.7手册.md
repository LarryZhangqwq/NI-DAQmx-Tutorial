...
**应用范例－硬件定时同步更新I/O，在实时和非实时循环之间交换数据**
...
![GUID-446D8BEE-8FFE-42A8-B941-DFF6CEA4247B-5.5x8.5 - a5](/Users/zhangyuxiang/Documents/NI-DAQmx-Tutorial/image/GUID-446D8BEE-8FFE-42A8-B941-DFF6CEA4247B-5.5x8.5 - a5.png)
...
![GUID-116218FE-55AA-4B42-AC70-516A4E887255-5.5x8.5 - a5](/Users/zhangyuxiang/Documents/NI-DAQmx-Tutorial/image/GUID-116218FE-55AA-4B42-AC70-516A4E887255-5.5x8.5 - a5.gif)
...
![GUID-358D8BA7-0704-410C-A2F3-74B9733B9589-5.5x8.5 - a5](/Users/zhangyuxiang/Documents/NI-DAQmx-Tutorial/image/GUID-358D8BA7-0704-410C-A2F3-74B9733B9589-5.5x8.5 - a5.png)
...
![GUID-31E08C53-03F9-43A6-AAD9-C832BA1EBFE7-5.5x8.5 - a5](/Users/zhangyuxiang/Documents/NI-DAQmx-Tutorial/image/GUID-31E08C53-03F9-43A6-AAD9-C832BA1EBFE7-5.5x8.5 - a5.gif)
...
![GUID-272ADDD9-5161-42E6-9895-2553F9077FD2-5.5x8.5 - a5](/Users/zhangyuxiang/Documents/NI-DAQmx-Tutorial/image/GUID-272ADDD9-5161-42E6-9895-2553F9077FD2-5.5x8.5 - a5.png)

##### 2.9.1.5 使用定时循环硬件定时同步更新I/O（仅限于LabVIEW）
...
通过任务控制循环定时源也允许用户指定采样时钟事件被处理和定时循环开始执行之间的时间延迟。这个延迟也称休眠时间，防止了“DAQmx读取”函数/VI占用全部CPU，并等待模拟输入样本被复用和数字化。

时序示意图

![GUID-90F4EF3E-CE54-4BA6-82BE-D29266B5B09B-5.5x8.5 - a5](/Users/zhangyuxiang/Documents/NI-DAQmx-Tutorial/image/GUID-90F4EF3E-CE54-4BA6-82BE-D29266B5B09B-5.5x8.5 - a5.png)
...
![GUID-E6C6D056-AE86-40A2-8592-456AAA578491-5.5x8.5 - a5](/Users/zhangyuxiang/Documents/NI-DAQmx-Tutorial/image/GUID-E6C6D056-AE86-40A2-8592-456AAA578491-5.5x8.5 - a5.gif)
...
![GUID-D0C1A02A-FC78-4A7B-8B89-35D7DF172D33-5.5x8.5 - a5](/Users/zhangyuxiang/Documents/NI-DAQmx-Tutorial/image/GUID-D0C1A02A-FC78-4A7B-8B89-35D7DF172D33-5.5x8.5 - a5.png)
...
- **解答**—在LabVIEW中使用定时循环或在LabWindows/CVI中使用异步计时器，实现软件定时。将NI-DAQmx任务配置为按需定时。
...
![GUID-417F8AB0-8D3F-45E0-8991-A63C28AFDA3D-5.5x8.5 - a5](/Users/zhangyuxiang/Documents/NI-DAQmx-Tutorial/image/GUID-417F8AB0-8D3F-45E0-8991-A63C28AFDA3D-5.5x8.5 - a5.png)
...
![GUID-3F93C932-2700-4F96-9B05-FE7B690C2275-5.5x8.5 - a5](/Users/zhangyuxiang/Documents/NI-DAQmx-Tutorial/image/GUID-3F93C932-2700-4F96-9B05-FE7B690C2275-5.5x8.5 - a5.gif)
...
控制循环有两种方式，通过软件控制和通过硬件控制。也可使用定时循环结构。

对于软件定时，软件和操作系统决定了循环执行的速率。软件定时不是确定性的定时。使用“等待下一个整数倍毫秒”VI来控制While循环是软件定时循环的一个范例。硬件定时是使用DAQ设备内部时钟或外部时钟在循环中控制何时进行读取操作。常见应用部分的*控制循环*的程序框图中使用的是硬件定时。

定时循环是硬件定时。定时循环是多速率应用的理想选择。默认情况下，定时循环使用Windows操作系统的1 kHz时钟作为时钟源。关于定时循环的详细信息，见*LabVIEW帮助*。

#### 2.9.3 控制算法

创建一个控制应用程序时需考虑多种数据处理算法。可使用LabVIEW创建自定义控制算法。也可使用VI处理控制应用程序的数据，例如，LabVIEW Control Design and Simulation模块的VI和函数。该模块提供用于设计、分析、仿真和部署动态系统模型和控制器的库和函数。

#### 2.9.4 同步模拟输入和输出

可安排模拟输出事件的时间，使其与模拟输入时钟保持同步。可在模数转换和数模转换上共享一个时钟，保证二者同步发生。该方法的好处是只要软件按时完成，就不会产生信号毛刺。建议将输出始终置于输入后一个周期，如下图所示。

![GUID-A192374F-2C01-470C-99B4-258BFFB20CDF-5.5x8.5 - a5](/Users/zhangyuxiang/Documents/NI-DAQmx-Tutorial/image/GUID-A192374F-2C01-470C-99B4-258BFFB20CDF-5.5x8.5 - a5.png)

#### 2.9.5 设置LabVIEW控制应用程序的优先级

在一个控制应用中，并行进程的相对优先级较为重要。由于LabVIEW支持多线程，因此可将应用程序分解为多个任务，每个任务具有不同的优先级。通过设置优先级，可安排实时任务在非实时任务前执行。实时任务必须定期将处理器资源释放给低优先级的任务，使其可以执行。通过对实时任务和低优先级任务进行合理分配，可以降低应用程序的抖动。关于分配任务优先级的更多信息见*LabVIEW Real-Time模块帮助*。

#### 2.9.6 I/O循环

一个采样时钟周期内的输入和输出操作以及相关的处理操作被称为*I/O循环*。I/O循环可以只包括读取和写入，但是典型控制应用中的I/O循环由读取操作、处理数据和写出结果三部分组成。



### 2.10 NI-DAQmx仿真设备

本文档包括NI-DAQmx设备的定时和触发、任务执行、读写数据的详细信息。

关于创建NI-DAQmx仿真设备和导入NI-DAQmx仿真设备配置至物理设备的详细信息，见*NI-DAQmx的MAX帮助*。

#### 2.10.1 NI-DAQmx仿真设备的定时和触发

用NI-DAQmx仿真设备时，需考虑下列注意事项：

- NI-DAQmx仿真设备模仿连续模拟输入、数字输入和所处任务的定时。计数器任务的定时无法仿真。
- NI-DAQmx仿真设备不会使定时循环开始执行。
- NI-DAQmx仿真设备支持软件事件。但是，不支持依赖于硬件的事件。例如，采样时钟事件。
- 触发立即开始。
- 看门狗定时器不过期。

#### 2.10.2 NI-DAQmx仿真设备的任务

使用NI-DAQmx仿真设备的NI-DAQmx任务的验证方式与实体设备任务的方式相同。如属性被设置未无效值，返回给NI-DAQmx仿真设备的错误与返回给实体设备的错误相同。任务必须的资源会被NI-DAQmx仿真设备保留。同实体设备类似，RTSI线缆、PXI触发线、DMA通道、计数器等都会被占用和预留。

**注**：同一任务中不能同时包括NI-DAQmx仿真设备和物理设备。

#### 2.10.3 使用NI-DAQmx仿真设备读取和写入数据
...
本章节包括部署应用程序和使用DAQmx I/O服务器实现分布式应用的详细信息。

#### 2.11.1 部署

*部署*是指开发一个应用程序，该程序开发完毕后，可发布、部署或下载至其他计算机。如要部署一个应用程序，需保存应用程序以及应用程序和系统所需的配置信息。

部署应用程序是，必须协调多个任务共享的配置项的使用。包括设备、换算和全局虚拟通道。

**开发用于部署的应用程序**

可通过下列方式部署NI-DAQmx应用程序：

- 使用MAX导出向导将整个设备部署至另一台计算机，包括任务、通道、换算和设备。
- 可使用MAX导出向导部署将整个设置（设备配置除外）至另一台计算机。如目标计算机已包含依赖于现有设备配置的任务，可选择该方式。在上述情况下，部署后可能需做相应的修改，使任务和通道引用至目标计算机的设备配置。
- 可使用MAX导出向导在一个用户组中部署特定数量的设备、换算和全局虚拟通道配置。组内每个成员都可创建基于共享配置的任务，然后创建使用这些任务的应用程序，最后在组内共享应用程序。在一些情况下，任务随应用程序一并自动部署。在另一些情况下，必须单独配置任务和应用程序。详细信息见部署任务和通道。

**部署任务和通道**

在下列情况下任务和通道随应用程序一并部署：

- DAQ应用程序使用LabVIEW Express VI，任务中只使用局部通道。
- 任务保存在LabWindows/CVI或Measurement Studio项目中，任务中只使用局部通道。
- 通过NI-DAQmx API创建任务和通道。

如通过下列方式创建任务和通道，则必须使用“MAX导出向导”部署任务和通道：

- 直接在MAX中创建任务和通道。
- 在LabVIEW中通过任务名称和通道名称控件创建任务和通道，且未生成配置代码。
- 在LabWindows/CVI中创建任务，且未生成配置代码或复制任务至项目。
- 创建的是包含全局虚拟通道的SignalExpress项目。
- 在DAQ应用程序中使用了LabVIEW Express VI，任务中使用全局虚拟通道，且未使用代码生成。

#### 2.11.2 DAQmx I/O服务器和虚拟通道

在NI-DAQmx 8.0或更高版本中，可设置分布式应用通过NI-DAQmx I/O服务器组合数据应用程序。例如，可配置一台中央计算机监测若干台通过共享变量控制硬件传感器的的计算机。可通过LabVIEW 8.0或更高版本连接NI-DAQmx I/O服务器。也可通过LabVIEW以外的途径连接NI-DAQmx I/O服务器。如使用第三方OPC客户端，还可访问DAQ通道。

**注**： 要使用NI-DAQmx I/O服务器，必须至少在项目中或MAX中定义一条全局虚拟通道。任何I/O类型的全局虚拟通道均可连接至共享变量，但是不是所有类型的任务都可连接至共享变量。

使用第三方OPC客户端时，连接至变量引擎服务器可访问网络上的DAQ通道。

关于在项目中使用共享变量捆绑DAQ通道的详细信息，见*LabVIEW帮助*。

### 2.12 电压生成的外部参考源

支持外部电压参考的设备使用户能够最大化设备的精度。如需生成的电压未超出一定电平并且可提供该电平的外部参考电压，将可实现该设备的最大精度。外部参考电压设置可作为**模拟输出»常规属性»DAC»参考电压**中的通道属性。

通过外部参考电压也可将增益应用至DC电压或随时间变化的波形。例如，设置外部参考电压电平为1.0，然后写入值范围为-1.0～+1.0 V的正弦波缓冲。当应用实际为2.0 V的参考电压时，信号的幅值将跳变为±2 V。增加参考电压电平为3.0将再次将信号跳变为±3 V。应用0.0 V的参考电压可立即将时变电压信号平整为0.0 V。
...
*换算前*表示以通道上的单位表示的值，未经过自定义换算。多数情况下，换算前单位是伏特或安培，因为通道测量或生成的信号多为电压或电流信号。但是，可在包含NI-DAQmx支持的传感器的通道上使用换算。在上述情况下，换算前单位就是所包含的传感器通道的单位。例如，可创建一个模拟输入电阻通道，并在该通道上使用一个换算。在该例中，换算前单位是欧姆，换算指定如何将欧姆转换为换算后单位。

*换算后*指NI-DAQmx应用自定义换算后的最终单位。例如，在电压输出通道上使用线性位置至电压转换的换算。在上述情况下，换算前单位是伏特，换算后单位是米。换算后的单位是最方便用于应用程序的单位。用户可完全控制换算后的单位。换算指定了通道测量或生成的信号从换算前单位到换算后单位的转换。当从与换算相关的通道读取样本时，得到的样本以换算后单位表示。同理，当写入样本至与换算相关的通道时，得到的样本以换算后单位表示。
...
NI-DAQmx设备注意事项包含以下设备的详细信息，以帮助用户使用NI-DAQmx创建应用程序：模拟输出(AO)系列设备、B系列设备、C系列设备、数字I/O (DIO)设备、DSA设备、E系列设备、FieldDAQ设备、M系列设备、网络DAQ设备、S系列设备、SCC设备、SC Express、SCXI设备、开关、定时I/O (TIO)设备、USB DAQ设备和X系列设备。

2018–2019 National Instruments. 版权所有

### 3.1 模拟触发

本章节包含用于C系列、NI ELVIS II系列、DSA、E系列、M系列、S系列和SC Express设备的模拟触发信息。

#### 3.1.1 C系列、E系列、M系列和S系列设备的模拟触发注意事项

**注**：部分E系列、M系列、S系列和C系列设备不支持模拟触发。查看设备的产品规范以确定设备是否支持模拟触发。

部分C系列、E系列、M系列和S系列设备包含单个可用于配置模拟触发的模拟触发电路。模拟触发电路属于设备的共享资源，任意子系统均可使用该资源。触发电路支持带滞后的电平和斜率触发以及模拟窗触发。进行配置后，输出电路将显示为“模拟比较事件”，该事件可以作为模拟输入、模拟输入和计数器子系统内不同触发和时钟的源。



**共享用于C系列、E系列、M系列和S系列设备的模拟触发器**

虽然模拟触发属于共享资源，但每次仅允许配置并保留一个模拟输入或模拟输出任务。如需在多个任务间共享模拟触发，可在单个任务中进行配置并保留，然后将触发、时钟或信号的源表示为模拟比较事件，从而在子系统中使用该触发。对于在相同任务中可支持多个模拟触发类型的任务，所有触发必须共享统统的配置，否则将发生错误。例如，如需将模拟触发用于单个模拟输入任务中的开始和参考触发，开始和参考触发的配置必须完全相同。
...
- PFI 0
  ...
  通常情况下，当配置模拟触发时，可将模拟触发信号连接至PFI 0接线端。由于PFI 0同时表示模拟和数字信号的触发源，当导出信号至接线端的任务不处于提交或运行状态时，NI-DAQmx会自动将接线端设置为三态。将信号导出至PFI 0的行为与其他PFI线的基于任务的连线有所不同。该行为可防止意外将模拟信号直接连接至数字电路，从而避免永久损坏设备。同时注意连接模拟信号至PFI 0时，接线端配置为参考单端。

  即便PFI 0不是模拟触发的源，由于启用模拟触发时将在设备内部替代PFI接线端，PFI 0将无法用于其他数字信号连线。如尝试同时使用模拟触发和用于数字信号的PFI 0，将返回一个连线错误。

  **注**：在PXI-6132/6133和PXIe-6124设备上，请勿使用PFI 0作为模拟触发源。PXI-6132/6133上，模拟触发电路仍预留PFI 0用于内部连线。

- 模拟输入通道

  除PFI 0之外，模拟输入任务可以触发任一进行采样的模拟输入通道。由于E系列设备使用扫描架构，当触发源为被采样的通道时，模拟触发的使用将存在限制。使用模拟开始触发时，触发通道必须为通道列表中的第一个通道。使用模拟参考或暂停触发时，并且模拟通道为触发源，通道列表中仅允许包含一路通道。如存在多个通道用于暂停或参考触发，必须使用PFI 0。由于S系列设备不使用扫描架构，这些限制均不适用。因此不管采样的通道数或队列中的通道顺序，S系列设备均将任意输入通道作为触发源。

- PFI 0换算和模拟输入通道

  如将PFI 0作为触发源，将无法应用换算（包括自定义换算）。例如，可将**DAQmx触发模拟边沿电平**属性的单位指定为伏特。但如将模拟输入通道作为触发源，即可使用换算后的单位。

**用于模拟触发的M系列有效源**

- APFI 0和APFI 1

  当配置模拟触发时，可将模拟信号连接至APFI 0或APFI 1并指定APFI 0或APFI 1作为触发源。

- 模拟输入通道

  除APFI 0和APFI 1之外，模拟输入任务可以触发任一进行采样的模拟输入通道。由于M系列设备使用扫描架构，当触发源为被采样的通道时，模拟触发的使用将存在限制。使用模拟开始触发时，触发通道必须为通道列表中的第一个通道。使用模拟参考或暂停触发时，并且模拟通道为触发源，通道列表中仅允许包含一路通道。如存在多个通道用于暂停或参考触发，必须使用APFI 0或APFI 1。

- APFI 0和APFI 1换算以及模拟输入通道

  如将APFI 0或APFI 1作为触发源，将无法应用换算（包括自定义换算）。例如，可将DAQmx触发模拟边沿电平属性的单位指定为伏特。但如将模拟输入通道作为触发源，将无法使用换算后的单位。

**用于E系列、M系列、C系列或S系列设备的模拟触发器的设备校准和精度**

通常情况下，E系列、M系列、C系列或S系列设备中模拟触发电路的触发DAC的精度比该设备的ADC小4位。模拟触发电路无可用硬件校准。此外如果触发信号包含一个高转换速率，从满足有效触发条件到模拟触发电路发出模拟比较事件的传播延迟可能会影响测量。如该条件对测量结果影响过大，通过将任务配置为正常并应用该模拟触发的已知信号，从而对模拟触发电路执行软件校准。通过比较测量结果和预期结果，可计算软件中需应用的必需偏移量以调整触发行为。

**用于模拟触发的C系列有效源**

NI 9205不包括APFI 0或APFI 1接线端。使用NI 9205的模拟输入任务可以触发NI 9205所采集的模拟输入通道。当使用模拟开始触发时，必须将通道列表中NI 9205的第一个通道作为触发通道，但也可使用其他来自C系列设备的通道。当使用模拟参考或暂停触发时，在通道列表中仅可使用NI 9205的一个通道，但可使用来自其他C系列设备的通道。通过多个NI 9205设备的不同配置可将模拟开始、参考和暂停触发进行组合。相同设备中所有模拟触发将共享相同的配置。

仅当模拟触发为活动通道时，才能为NI 9775使用。关于模拟触发用法的更多限制条件，见*NI 9775注意事项*。如NI 9775使用模拟多边沿触发，所有触发通道必须来自同一设备。如使用多个NI 9775模块，只要触发源来自其中一个NI 9775模块，这些模块就可用于同一任务。

### 3.2 设备校准注意事项

*设备校准*包括检验设备的测量精度以及调整测量错误。验证包括测量设备性能并将测量结果与已发布的规范进行比较。在校准过程中，通过外部标准可提供并读取电压电平以及其它信号，然后调整设备的校准常量。新的校准常量将被保存在EEPROM中。根据需要可从内存中加载校准常量并调整设备测量中的误差。

#### 3.2.1 C系列校准

该设备将通过软件校准对该设备读取或生成的信号的软件换算进行调整。通过精准的参考信号，设备测量并计算模拟输入和模拟输出的换算常量。该换算常量将保存在设备的非易失性存储器(EEPROM)中。NI建议在开启模块并预热至少15分钟后，在开始测量前对设备进行校准。对该设备进行校准时必须使用相同的预热时间。多次校准可确保更稳定和可重复的校准性能。经常进行重新校准并不会影响该设备。

**注**：

- 校准该设备可能需要等待一定时间。如“调整C系列校准”函数/VI需几秒钟后才执行，请耐心等待。
- 如需获取最优结果，请在开始校准之前停止其它正在运行的任务并断开无用的外部连接。
- NI 9205和NI 9206按照M系列校准过程执行校准。

#### 3.2.2 FieldDAQ校准

FieldDAQ设备通过软件校准对设备读取或生成的信号的软件换算进行调整。通过精准的参考信号，设备测量并计算模拟输入的换算常量。该换算常量将保存在设备的非易失性存储器(EEPROM)中。多次校准可确保更稳定和可重复的校准性能。经常进行重新校准并不会影响该设备。

**注**：

- 校准该设备可能需要等待一定时间。如“调整FieldDAQ校准”函数/VI需几秒钟后才执行，请耐心等待。
- 为得到最优结果，请在开始校准之前停止其他正在运行的任务并断开无用的外部连接。

FieldDAQ系列设备不会自校准或自动校准。必须手动校准此类设备。

FieldDAQ设备包括FD-11601、FD-11603、FD-11605、FD-11613、FD-11614、FD-11634和FD-11637。

### 3.3 计数器

本章节介绍计数器信号连接和内部计数器连线的连线图。

#### 3.3.1 支持平均

下列设备可支持平均数字频率和周期测量：

- 连接到CompactDAQ机箱[[1\]](https://www.ni.com/documentation/zhs/ni-daqmx/20.1/devconsid/averagingcounterinput/#note-1)、CompactDAQ控制器[[2\]](https://www.ni.com/documentation/zhs/ni-daqmx/20.1/devconsid/averagingcounterinput/#note-2)、CompactRIO控制器[[3\]](https://www.ni.com/documentation/zhs/ni-daqmx/20.1/devconsid/averagingcounterinput/#note-3)，以及 CompactRIO单板控制器[[4\]](https://www.ni.com/documentation/zhs/ni-daqmx/20.1/devconsid/averagingcounterinput/#note-4)的并行数字I/O模块。请参考*C系列设备的数字I/O注意事项*。
- NI 661*x*设备
- X系列设备

- [1](https://www.ni.com/documentation/zhs/ni-daqmx/20.1/devconsid/averagingcounterinput/#note_ref-1) cDAQ-9171、9174、9178、9179、9181、9184、9185、9188、9188XT、9189和9191。
- [2](https://www.ni.com/documentation/zhs/ni-daqmx/20.1/devconsid/averagingcounterinput/#note_ref-2) cDAQ-9132、9133、9134、9135、9136和9137。
- [3](https://www.ni.com/documentation/zhs/ni-daqmx/20.1/devconsid/averagingcounterinput/#note_ref-3) cRIO-9040、9041、9042、9043、9045、9046、9047、9048、9049、9053、9054、9055、9056、9057和9058。
- [4](https://www.ni.com/documentation/zhs/ni-daqmx/20.1/devconsid/averagingcounterinput/#note_ref-4) sbRIO-9603、9608、9609、9628、9629和9638。

#### 3.3.2 C系列计数器模块

NI 9361计数器输入模块支持下列测量类型：
...
NI 9361有8个计数器，支持向同一任务添加多个计数器通道。单个任务可包含不同测量类型，同一任务可添加多个计数器模块通道。

**注：**

- 同一任务不可混用模块计数器和机箱计数器。
- 不可通过计数器模块使用机箱计数器。
- 不可使用单个NI 9361创建多计数器任务。
- 一个机箱中不能包含两个以上使用不同定时/触发配置的NI 9361。

#### 3.3.3 连接计数器信号

不同设备中用于计数器测量和生成的默认接线端可能有所不同。关于特定设备的详细信息见以下链接。如需忽略默认输入接线端，可根据测量类型设置DAQmx通道输入接线端属性。例如，对边沿进行计数时，可使用CI.CountEdges.Term。如需忽略默认输出接线端，可将DAQmx通道输出接线端属性设置为所需值。

##### 3.3.3.1 计数器的C系列信号连接

下表包含用于各种计数器测量的默认输入接线端。任意输入接线端均可使用不同的PFI线。利用NI-DAQmx通道属性可更改测量的PFI输入。

有关CompactRIO单板控制器上的计数器的信息，请参阅*CompactRIO单板控制器物理通道*的计数器输入和输出部分。

**NI 9401、NI 9421、NI 9422、NI 9423、NI 9436和NI 9437（8通道）**

| 测量                                | Ctr0                       | Ctr1                       | Ctr2                       | Ctr3                       |
| :---------------------------------- | :------------------------- | :------------------------- | :------------------------- | :------------------------- |
| 边沿计数                            | 边沿：PFI 0计数方向：PFI 2 | 边沿：PFI 4计数方向：PFI 6 | 边沿：PFI 3计数方向：PFI 0 | 边沿：PFI 7计数方向：PFI 4 |
| 脉冲宽度测量                        | PFI 1                      | PFI 5                      | PFI 2                      | PFI 6                      |
| 周期/频率测量（单个计数器的低频）   | PFI 1                      | PFI 5                      | PFI 2                      | PFI 6                      |
| 周期/频率测量（两个计数器的高频）   | PFI 0                      | PFI 4                      | PFI 3                      | PFI 7                      |
| 周期/频率测量（两个计数器的宽量程） | PFI 0                      | PFI 4                      | PFI 3                      | PFI 7                      |
| 半周期测量                          | PFI 1                      | PFI 5                      | PFI 2                      | PFI 6                      |
| 两边沿间隔测量                      | 开始：PFI 2停止：PFI 1     | 开始：PFI 6停止：PFI 5     | 开始：PFI 0停止：PFI 2     | 开始：PFI 4停止：PFI 6     |
| 位置测量                            | A: PFI 0B: PFI 2Z: PFI 1   | A: PFI 4B: PFI 6Z: PFI 5   | A: PFI 3B: PFI 0Z: PFI 2   | A: PFI 7B: PFI 4Z: PFI 6   |

**NI 9402、NI 9435（4通道）**

| 测量                                | Ctr0                       | Ctr1                       | Ctr2                       | Ctr3                       |
| :---------------------------------- | :------------------------- | :------------------------- | :------------------------- | :------------------------- |
| 边沿计数                            | 边沿：PFI 0计数方向：PFI 2 | 边沿：PFI 3计数方向：PFI 1 | 边沿：PFI 1计数方向：PFI 0 | 边沿：PFI 2计数方向：PFI 3 |
| 脉冲宽度测量                        | PFI 1                      | PFI 2                      | PFI 3                      | PFI 0                      |
| 周期/频率测量（单个计数器的低频）   | PFI 1                      | PFI 2                      | PFI 3                      | PFI 0                      |
| 周期/频率测量（两个计数器的高频）   | PFI 0                      | PFI 3                      | PFI 1                      | PFI 2                      |
| 周期/频率测量（两个计数器的宽量程） | PFI 0                      | PFI 3                      | PFI 1                      | PFI 2                      |
| 半周期测量                          | PFI 1                      | PFI 2                      | PFI 3                      | PFI 0                      |
| 两边沿间隔测量                      | 开始：PFI 2停止：PFI 1     | 开始：PFI 1停止：PFI 2     | 开始：PFI 0停止：PFI 3     | 开始：PFI 3停止：PFI 0     |
| 位置测量                            | A: PFI 0B: PFI 2Z: PFI 1   | A: PFI 3B: PFI 1Z: PFI 2   | A: PFI 1B: PFI 0Z: PFI 3   | A: PFI 2B: PFI 3Z: PFI 0   |

**NI 9411（6通道）**

| 测量                                | Ctr0                       | Ctr1                       | Ctr2                       | Ctr3                       |
| :---------------------------------- | :------------------------- | :------------------------- | :------------------------- | :------------------------- |
| 边沿计数                            | 边沿：PFI 0计数方向：PFI 2 | 边沿：PFI 3计数方向：PFI 5 | 边沿：PFI 2计数方向：PFI 1 | 边沿：PFI 5计数方向：PFI 4 |
| 脉冲宽度测量                        | PFI 1                      | PFI 4                      | PFI 0                      | PFI 3                      |
| 周期/频率测量（单个计数器的低频）   | PFI 1                      | PFI 4                      | PFI 0                      | PFI 3                      |
| 周期/频率测量（两个计数器的高频）   | PFI 0                      | PFI 3                      | PFI 2                      | PFI 5                      |
| 周期/频率测量（两个计数器的宽量程） | PFI 0                      | PFI 3                      | PFI 2                      | PFI 5                      |
| 半周期测量                          | PFI 1                      | PFI 4                      | PFI 0                      | PFI 3                      |
| 两边沿间隔测量                      | 开始：PFI 2停止：PFI 1     | 开始：PFI 5停止：PFI 4     | 开始：PFI 1停止：PFI 0     | 开始：PFI 4停止：PFI 3     |
| 位置测量                            | A: PFI 0B: PFI 2Z: PFI 1   | A: PFI 3B: PFI 5Z: PFI 4   | A: PFI 2B: PFI 1Z: PFI 0   | A: PFI 5B: PFI 4Z: PFI 3   |

下表包含用于计数器输出的输出接线端。任意输出接线端均可使用不同的PFI线。

**NI 9401、NI 9472、NI 9474、NI 9475和NI 9485（8通道）**

| Ctr0  | Ctr1  | Ctr2  | Ctr3  | FreqOut |
| :---- | :---- | :---- | :---- | :------ |
| PFI 3 | PFI 7 | PFI 1 | PFI 5 | PFI 2   |

**NI 9481、NI 9482、NI 9402（4通道）**
...
| 测量                      | Ctr0                                 | Ctr1                                 | Ctr2                                 | Ctr3                                 | Ctr4                                 | Ctr5                                 | Ctr6                                 | Ctr7                                 |
| :------------------------ | :----------------------------------- | :----------------------------------- | :----------------------------------- | :----------------------------------- | :----------------------------------- | :----------------------------------- | :----------------------------------- | :----------------------------------- |
| 边沿计数                  | 边沿：PFI 0计数方向：PFI 7重置：PFI4 | 边沿：PFI 1计数方向：PFI 6重置：PFI5 | 边沿：PFI 2计数方向：PFI 5重置：PFI6 | 边沿：PFI 3计数方向：PFI 4重置：PFI7 | 边沿：PFI 4计数方向：PFI 3重置：PFI0 | 边沿：PFI 5计数方向：PFI 2重置：PFI1 | 边沿：PFI 6计数方向：PFI 1重置：PFI2 | 边沿：PFI 7计数方向：PFI 0重置：PFI3 |
| 占空比测量                | PFI 0                                | PFI 1                                | PFI 2                                | PFI 3                                | PFI 4                                | PFI 5                                | PFI 6                                | PFI 7                                |
| 脉冲宽度测量              | PFI 0                                | PFI 1                                | PFI 2                                | PFI 3                                | PFI 4                                | PFI 5                                | PFI 6                                | PFI 7                                |
| 周期/频率测量（动态平均） | PFI 0                                | PFI 1                                | PFI 2                                | PFI 3                                | PFI 4                                | PFI 5                                | PFI 6                                | PFI 7                                |
| 位置测量                  | A: PFI 0B: PFI 4Z: PFI 7             | A: PFI 1B: PFI 5Z: PFI 6             | A: PFI 2B: PFI 6Z: PFI 5             | A: PFI 3B: PFI 7Z: PFI 4             | A: PFI 4B: PFI 0Z: PFI 3             | A: PFI 5B: PFI 1Z: PFI 2             | A: PFI 6B: PFI 2Z: PFI 1             | A: PFI 7B: PFI 3Z: PFI 0             |
| 速度                      | A: PFI 0B: PFI 4                     | A: PFI 1B: PFI 5                     | A: PFI 2B: PFI 6                     | A: PFI 3B: PFI 7                     | A: PFI 4B: PFI 0                     | A: PFI 5B: PFI 1                     | A: PFI 6B: PFI 2                     | A: PFI 7B: PFI 3                     |

#### 3.3.4 计数器内部连线图

本部分介绍下列设备内部的计数器连线框图：AO系列、用于CompactDAQ机箱的C系列、E系列、M系列、S系列、TIO和X系列。

##### 3.3.4.1 NI cDAQ-91xx机箱上C系列设备内部连线图

下图为在cDAQ-9132、9133、9134、9135、9136、9137、9171、9174、9178、9179、9181、9184、9185、9188、9188XT、9189和9191机箱中使用的C系列设备的内部连线。该图显示的是可用的直接路径，也可使用间接路径，但需要消耗额外的内部资源。黑色圆圈表示接线端。
...
DAQmx－数据采集节点与NI-DAQmx硬件设备用于开发仪器、采集和控制应用程序。关于NI-DAQmx支持设备的完整列表，见NI-DAQmx自述文件。

### 4.1 DAQmx创建虚拟通道

创建单个或一组虚拟通道，并将其添加至任务

在循环中使用该节点时，如未指定任务输入，软件将在循环的每次迭代中创建新的任务。完成任务前，应在循环中使用“DAQmx清除任务”节点，以避免不必要的内存分配。
...
该模式用于需要激励的自定义传感器。激励可用于换算测量。



##### ![DAQmx Create Channel (AI-Acceleration-4 Wire DC Voltage).gvi](/Users/zhangyuxiang/Documents/NI-DAQmx-Tutorial/image/DAQmx Create Channel (AI-Acceleration-4 Wire DC Voltage).gvi.png)

**输入输出**

**任务输入**指定要添加节点创建的虚拟通道的任务。如没有指定任务，NI-DAQmx将自行创建任务并将节点创建的虚拟通道添加至该任务。

**物理通道**指定用于创建虚拟通道的物理通道名称。DAQmx物理通道常量将列出系统已安装的设备或模块中的所有物理通道。

也可以为该输入连接包含物理通道列表或范围的字符串。通过“DAQmx平化通道字符串”节点可将物理通道数组转换为列表。

**分配名称**是分配给VI创建的虚拟通道的名称。如该输入端未连线，NI-DAQmx将把物理通道名称作为虚拟通道名称。

如将自定义的虚拟通道名称连接至该输入端，在其它NI-DAQmx节点（例如，“DAQmx触发节点”的**源**输入端）中引用这些通道时，必须使用自定义名称。

对于使用“DAQmx创建虚拟通道”节点创建的多个虚拟通道，通过用逗号分隔的列表可为虚拟通道指定名称。如输入的名称数量少于创建的虚拟通道的数量，NI-DAQmx将为虚拟通道自动分配名称。

**单位**指定从通道返回的加速度测量所使用的单位。

| 名称              | 说明                                                         |
| :---------------- | :----------------------------------------------------------- |
| From Custom Scale | 自定义换算指定的单位。如选择该值，必须将自定义换算的名称连接至**自定义换算名称**输入端。 |
| g                 | 重力加速度g约等于9.81 m/s/s。                                |
| in/s^2^           | 英寸每二次方秒。                                             |
| m/s^2^            | 米每二次方秒。                                               |

**错误输入**表明节点运行前发生的错误。该输入提供标准“错误输入”。

**测量范围**指定预期测量的最大值和最小值。

* **最大值**按设定的**单位**，指定测量预期的最大值。
* **最小值**按设定的**单位**，指定测量预期的最小值。

**输入接线端配置**指定通道的输入接线端配置。

| 名称               | 说明                                         |
| :----------------- | :------------------------------------------- |
| default            | 运行时，NI-DAQmx将为通道选择默认接线端配置。 |
| Differential       | 差分模式。                                   |
| NRSE               | 非参考单端模式。                             |
| Pseudodifferential | 伪差分模式。                                 |
| RSE                | 参考单端模式。                               |
...
FPGA: LabVIEW NXG不支持FPGA设备

Web服务器: 不支持在Web应用程序VI中使用

##### 4.1.1.2 加速度 (G数据流)

创建通道，使用加速计测量加速度。

![DAQmx Create Channel (AI-Acceleration-Accelerometer).gvi](/Users/zhangyuxiang/Documents/NI-DAQmx-Tutorial/image/DAQmx Create Channel (AI-Acceleration-Accelerometer).gvi.png)
...
**自定义换算名称**指定用于通道的自定义换算的名称。

**灵敏度**指传感器的灵敏度。值的单位由**灵敏度单位**指定。关于确定该值的详细信息见相关传感器文档。
...
**任务输出**是节点执行结束后对任务的引用。任务中包含全部新建的虚拟通道。**任务输入**没有连线时，NI-DAQmx将自动创建该输出引用的任务。

**电流激励**指定了激励的源。

| 名称     | 说明                                                         |
| :------- | :----------------------------------------------------------- |
| External | 使用设备的内置激励源之外的任何激励源。选择该值后，必须通过**电流激励值**输入端指定激励。 |
| Internal | 使用设备内置的激励源。选择该值后，必须通过**电流激励值**输入端指定激励。 |
| None     | 不向通道提供激励。                                           |

**电流激励值**指定为传感器提供的激励总量，以安培为单位。关于确定该值的详细信息见相关传感器文档。

**错误输出**包含错误信息。如**错误输入**表明该节点运行前已发生错误，则**错误输出**将包含相同的错误信息。否则，**错误输出**表示该节点产生的错误状态。

##### 4.1.1.3 电荷(G数据流)

创建使用基于电荷的传感器通道测量加速度。

![DAQmx Create Channel (AI-Acceleration-Charge).gvi](/Users/zhangyuxiang/Documents/NI-DAQmx-Tutorial/image/DAQmx Create Channel (AI-Acceleration-Charge).gvi.png)
...
创建通道，使用惠斯通电桥测量电压比率。通过该实例使用基于电桥的传感器除应变、力、压力或扭矩以外的现象，或换算数据至NI-DAQmx不支持的物理单位。

通过自定义换算或其他换算代码，可将电压比率位转换为物理单位。

![DAQmx Create Channel (AI-Bridge).gvi](/Users/zhangyuxiang/Documents/NI-DAQmx-Tutorial/image/DAQmx Create Channel (AI-Bridge).gvi.png)
...
* **桥配置**指定连接至通道的惠斯通电桥配置。
  ...
  | 名称           | 说明                         |
  | :------------- | :--------------------------- |
  | Full Bridge    | 电桥包含四个有效传感计元素。 |
  | Half Bridge    | 电桥包含两个有效传感计元素。 |
  | Quarter Bridge | 电桥包含一个有效传感计元素。 |

* **电压激励源**指定了激励的源。

  | 名称     | 说明                                                         |
  | :------- | :----------------------------------------------------------- |
  | External | 使用设备的内置激励源之外的任何激励源。选择该值后，必须通过**电压激励值**输入端指定激励值。 |
  | Internal | 使用设备内置的激励源。选择该值后，必须通过**电压激励值**输入端指定激励值。 |
  | None     | 不向通道提供激励。                                           |

* **电压激励值**指定为传感器提供的激励总量，以伏特为单位。关于确定适当激励值的更多信息，见传感器相关文档。

* **额定桥电阻**指定无负载时电桥的电阻，以欧姆为单位。
...
![DAQmx Create Channel (AI-Charge).gvi](/Users/zhangyuxiang/Documents/NI-DAQmx-Tutorial/Image/DAQmx Create Channel (AI-Charge).gvi.png)
...
![DAQmx Create Channel (AI-Current-Basic).gvi](/Users/zhangyuxiang/Documents/NI-DAQmx-Tutorial/Image/DAQmx Create Channel (AI-Current-Basic).gvi.png)
...
**分流电阻位置**指定了分流电阻的位置。对于带有内置分流电阻的设备，位置可指定为**内部**。对于没有内置分流电阻的设备，必须连接一个外部分流电阻。使用外部分流电阻时，可通过将位置指定为**外部**，使用**外部分流电阻值**输入端指定电阻的值。

| 名称     | 说明                                                         |
| :------- | :----------------------------------------------------------- |
| default  | 对于带有内置分流电阻的设备，NI-DAQmx将把输入值设置为**内部**。对于没有内置分流电阻的设备，NI-DAQmx将把输入值设置为**外部**。 |
| External | 使用设备外部的分流电阻。必须通过**外部分流电阻值**输入端指定分流电阻的值。 |
| Internal | 使用设备内置的分流电阻。                                     |

**任务输出**是节点执行结束后对任务的引用。任务中包含全部新建的虚拟通道。**任务输入**没有连线时，NI-DAQmx将自动创建该输出引用的任务。

**外部分流电阻值**指定外部分流电阻的值，以欧姆为单位。

**错误输出**包含错误信息。如**错误输入**表明该节点运行前已发生错误，则**错误输出**将包含相同的错误信息。否则，**错误输出**表示该节点产生的错误状态。

##### 4.1.1.7 RMS

创建通道，测量电流RMS（采样电流的平均功率）。

![DAQmx Create Channel (AI-Current-RMS).gvi](/Users/zhangyuxiang/Documents/NI-DAQmx-Tutorial/Image/DAQmx Create Channel (AI-Current-RMS).gvi.png)
...
创建通道，**使用惠斯通电桥测量力或负载**。该实例适用于规范包含了转换电学值至物理值所需多项式的传感器。使用该换算类型时，NI-DAQmx通过多项式系数换算电学值为物理值（前向）及换算物理值为电学值（反向）。如仅有一组系数已知，可通过“DAQmx计算反向多项式系数”节点生成另一组系数。

如传感器规范使用的物理单位与NI-DAQmx换算的单位不一致，可为**单位**和**物理单位**指定不同的值。

![DAQmx Create Channel (AI-Force-Bridge-Polynomial).gvi](/Users/zhangyuxiang/Documents/NI-DAQmx-Tutorial/Image/DAQmx Create Channel (AI-Force-Bridge-Polynomial).gvi.png)
...
**换算信息**指定如何将传感器的电信号转换为物理信号。

* **前向系数**为多项式的系数数组，该多项式可用于将电学单位转换为物理单位。数组中的每个元素都对应于公式中的一个项。例如，如果数组的索引3为9，方程的第4项为9x^3^。

* **反向系数**为多项式的系数数组，该多项式可用于将物理单位转换为电学单位。数组中的每个元素都对应于公式中的一个项。例如，如果数组的索引3为9，方程的第4项为9x^3^。

* **电学单位**指定用于换算数据的电学单位。选择与传感器数据表或校准认证用于电学值相同的单位。
...
* **物理单位**指定用于换算电学数据的物理单位。选择与传感器数据表或校准认证用于物理值相同的单位。
...
创建通道，**使用惠斯通电桥测量力或负载**。该实例适用于规范为电学值和对应物理值表格的传感器。使用该换算类型时，NI-DAQmx在电学值和对应物理值间进行线性换算。**最小值**和**最大值**指定的输入限制必须位于最小和最大物理值之间。对于超出范围的数据，NI-DAQmx会将数据转换为终止端点。

如传感器规范使用的物理单位与NI-DAQmx换算的单位不一致，可为**单位**和**物理单位**指定不同的值。

##### ![DAQmx Create Channel (AI-Force-Bridge-Table).gvi](Image/DAQmx Create Channel (AI-Force-Bridge-Table).gvi.png)
...
* **电学值**是由电学值组成的数组，值对应于**物理值**。以**电学单位**为单位。
...
* **物理值**是由物理值组成的数组，值对应于**电学值**。以**物理单位**为单位。
...
创建通道，使用惠斯通电桥测量力或负载。**该实例适用于规范未提供换算所需多项式，或包含电学值和对应物理值表格的传感器**。使用该换算类型时，NI-DAQmx通过两个电学值和物理值，计算线性方程的斜率和y截距，该方程可用于将电学单位换算为物理单位。

如传感器规范使用的物理单位与NI-DAQmx换算的单位不一致，可为**单位**和**物理单位**指定不同的值。

![DAQmx Create Channel (AI-Force-Bridge-Two-Point-Linear).gvi](Image/DAQmx Create Channel (AI-Force-Bridge-Two-Point-Linear).gvi.png)
...
* **第一个电学值**对应于**第一个物理值**的电学值。以**电学单位**为单位。

* **第二个电学值**对应于**第二个物理值**的电学值。以**电学单位**为单位。
...
* **第一个物理值**对应于**第一个电学值**的物理值。以**物理单位**为单位。

* **第二个物理值**对应于**第二个电学值**的物理值。以**物理单位**为单位。
...
**创建通道，使用IEPE力传感器测量力或负载。**

![DAQmx Create Channel (AI-Force-IEPE Sensor).gvi](Image/DAQmx Create Channel (AI-Force-IEPE Sensor).gvi.png)
...
![DAQmx Create Channel (AI-Frequency-Voltage).gvi](Image/DAQmx Create Channel (AI-Frequency-Voltage).gvi.png)
...
**阈值电平**指定用于识别波形重复的电平，以伏特为单位。必须选择在整个波形周期中仅出现一次的电压电平。也可以选择在电压上升或下降时仅出现一次的电压。

**滞后**指定**电平**下的窗，以伏特为单位。NI-DAQmx在识别波形重复前，输入电压必须小于**阈值电平**和**滞后**的差。信号含有噪声或抖动时滞后可提高测量的准确性。
...
![DAQmx Create Channel (AI-Position-EddyCurrentProxProbe).gvi](Image/DAQmx Create Channel (AI-Position-EddyCurrentProxProbe).gvi.png)
...
| 名称          | 说明            |
| :------------ | :-------------- |
| mVolts/mil    | mVolts/mil。    |
| Volts/mil     | Volts/mil。     |
| mVolts/mm     | mVolts/mm。     |
| Volts/mm      | Volts/mm。      |
| mVolts/micron | mVolts/micron。 |
...
![DAQmx Create Channel (AI-Position-LVDT).gvi](Image/DAQmx Create Channel (AI-Position-LVDT).gvi.png)
...
**电压激励连线模式**指传感器上引线的数量。部分传感器要求连接导线以创建4线或5线传感器。详细信息见相关传感器文档。
...
| 名称                   | 说明                  |
| :--------------------- | :-------------------- |
| mVolts/Volt/0.001 Inch | 毫伏/伏特/0.001英寸。 |
| mVolts/Volt/mMeter     | 毫伏/伏特/毫米。      |
...
**电压激励频率**指定了传感器要求的激励频率，以赫兹为单位。关于确定该值的详细信息见相关传感器文档。
...
![DAQmx Create Channel (AI-Position-RVDT).gvi](Image/DAQmx Create Channel (AI-Position-RVDT).gvi.png)
...
| 名称               | 说明             |
| :----------------- | :--------------- |
| mVolts/Volt/Degree | mV/V/角度。      |
| mVolts/Volt/Radian | 毫伏/伏特/弧度。 |
...
创建通道，**使用惠斯通电桥测量压力**。该实例适用于规范包含了转换电学值至物理值所需多项式的传感器。使用该换算类型时，NI-DAQmx通过多项式系数换算电学值为物理值（前向）及换算物理值为电学值（反向）。如仅有一组系数已知，可通过“DAQmx计算反向多项式系数”节点生成另一组系数。

如传感器规范使用的物理单位与NI-DAQmx换算的单位不一致，可为**单位**和**物理单位**指定不同的值。

![DAQmx Create Channel (AI-Pressure-Bridge-Polynomial).gvi](Image/DAQmx Create Channel (AI-Pressure-Bridge-Polynomial).gvi.png)
...
![DAQmx Create Channel (AI-Resistance).gvi](Image/DAQmx Create Channel (AI-Resistance).gvi.png)
...
![DAQmx Create Channel (AI-Sound Pressure-Microphone).gvi](Image/DAQmx Create Channel (AI-Sound Pressure-Microphone).gvi.png)
...
**最大声压**是可测量的最大即时声压。该值单位为分贝，参考20 MPa。
...
![DAQmx Create Channel (AI-Strain-Rosette Strain Gage).gvi](Image/DAQmx Create Channel (AI-Strain-Rosette Strain Gage).gvi.png)

**任务输入**指定要添加节点创建的虚拟通道的任务。如没有指定任务，NI-DAQmx将自行创建任务并将节点创建的虚拟通道添加至该任务。

**物理通道**指定物理通道的名称，用来创建计算**花型测量**通道所需的应变计虚拟通道。

对于每个应变花传感器，节点在应变花中为每个应变计创建一个应变虚拟通道，为**花型测量**数组中指定的每个测量创建一个应变花虚拟通道。**T型花**至少需要2条物理通道。**直角花**和**三角花**至少需要3条物理通道。如要为多个花型传感器串讲虚拟通道，必需有所需物理通道的整数倍条物理通道。通道的顺序对于花型应变计测量至关重要，应该与花型应变计的顺序保持一致。DAQmx要求应变计的标准通常为A、B、C，或者1、2、3。DAQmx物理通道常量将列出系统已安装的设备或模块中的所有物理通道。也可以为该输入连接包含物理通道列表或范围的字符串。通过“DAQmx平化通道字符串”节点可将物理通道数组转换为列表。

**分配名称**是分配给VI创建的虚拟通道的名称。如不连接该输入，NI-DAQmx将创建一个默认的通道名称。

名称的格式为rosette#_TYPE，#是唯一的递增值，TYPE是通道类型。

| rosette#_strainGage1            | 应变计1。        |
| ------------------------------- | ---------------- |
| rosette#strainGage2             | 应变计2。        |
| rosette#strainGage3             | 应变计3。        |
| rosette#principalStrainAngle    | 主应变角。       |
| rosette#cartesianStrainX        | 笛卡尔应变X。    |
| rosette#cartesianStrainY        | 笛卡尔应变Y。    |
| rosette#cartesianShearStrainXY  | 笛卡尔剪应变XY。 |
| rosette#maximumShearStrain      | 最大剪应变。     |
| rosette#maximumShearStrainAngle | 最大剪应变角。   |

如将自定义的虚拟通道名称连接至该输入端，在其他NI-DAQmx节点（例如，“DAQmx触发节点”的**源**输入端）中引用这些通道时，必须使用自定义名称。
...
* **花型类别**指定花型应变的类型。
  ...
  | 名称                | 说明                                        |
  | :------------------ | :------------------------------------------ |
  | Rectangular Rosette | 直角花包含三个应变计，互相之间呈45度角。    |
  | Delta Rosette       | 三角花包含三个应变计，互相之间呈60度角。    |
  | Tee Rosette         | T型应变花包含两个应变计，互相之间呈90度角。 |

* **应变片方向**指定应变相对于推荐X轴的方向，以角度为单位。该参数仅适用于笛卡尔测量。

* **花型测量**指定用于应变花测量而创建的虚拟通道。对于每个应变花传感器，节点在应变花中为每个应变计创建一个应变虚拟通道，为花型测量数组中指定的每个测量创建一个应变花虚拟通道。

  | 名称                       | 说明                                                         |
  | :------------------------- | :----------------------------------------------------------- |
  | principal strain 1         | 与受压材料表面共面的拉伸应变最大值。                         |
  | principal strain 2         | 与受压材料表面共面的拉伸应变最小值。                         |
  | principal strain angle     | 花型感应器的应变角与X轴的相对角度。范围是-180度到180度之间，负值表示在X轴下方。对于T型应变，值为0度。 |
  | cartesian strain X         | X坐标方向上与受压材料表面共面的拉伸应变。                    |
  | cartesian strain Y         | Y坐标方向上与受压材料表面共面的拉伸应变。                    |
  | cartesian shear strain XY  | XY坐标方向上与受压材料表面共面的应变。                       |
  | maximum shear strain       | 与受压材料横截面共面的拉伸应变最大值。                       |
  | maximum shear strain angle | 花型感应器最大剪应变角与X轴的相对角度。范围是-125度到225度之间，负值表示在X轴下方。最大剪应变发生在与主应变角呈45度偏移时。对于T型应变，值为45度。 |
...
* **注：** 常见的花型传感器使用**四分之一桥I**应变配置

* | 名称              | 说明       |
  | :---------------- | :--------- |
  | Full Bridge I     | 全桥 I。   |
  | Full Bridge II    | 全桥 II。  |
  | Full Bridge III   | 全桥 III。 |
  | Half Bridge I     | 半桥 I。   |
  | Half Bridge II    | 半桥 II。  |
  | Quarter Bridge I  | 1/4桥 I。  |
  | Quarter Bridge II | 1/4桥 II。 |
...
* **导线电阻**是导线的电阻，以欧姆为单位。理想情况下，导线的电阻相同。

**应变计信息**包含关于应变计和测量的信息。

* **应变计因子**指定应变计的敏感度，将电阻变换转换为应变的变换。电桥中的每个应变计都必须具有相同的应变计因子。关于确定该值的详细信息见相关传感器文档。
* **额定应变计电阻**是应变计处于非应变状态时的电阻，以欧姆为单位。电桥中的每个应变计都必须具有相同的额定应变计电阻。无应变计的电桥臂之间的电阻也必须与额定应变计电阻相同。关于确定该值的详细信息见相关传感器文档。
* **泊松比**指定测量材料横向应变和纵向应变的比。

**测量范围**指定预期测量的最大值和最小值。

* **最大值**指定预期测量的最大应变。该值将应用于花型应变结构中的每个应变计。
* **最小值**指定预期测量的最小应变。该值将应用于花型应变结构中的每个应变计。
...
![DAQmx Create Channel (AI-Strain-Strain Gage).gvi](Image/DAQmx Create Channel (AI-Strain-Strain Gage).gvi.png)
...
* **初始电桥电压**指定在无负载条件下电桥的输出电压，以欧姆为单位。在应用换算方程前，NI-DAQmx会从测量值中减去该值。对电桥进行无应变电压测量。
...
创建通道，使用接线盒或其他设备的内置传感器测量温度。例如，SCXI模块上的内置CJC传感器。

![DAQmx Create Channel (AI-Temperature-Built-in Sensor).gvi](Image/DAQmx Create Channel (AI-Temperature-Built-in Sensor).gvi.png)
...
![DAQmx Create Channel (AI-Temperature-RTD).gvi](Image/DAQmx Create Channel (AI-Temperature-RTD).gvi.png)
...
| 名称   | 说明                                                         |
| :----- | :----------------------------------------------------------- |
| Custom | 必须通过“模拟输入:温度:RTD:自定义:A”、“模拟输入:温度:RTD:自定义:B”和“模拟输入:温度:RTD:自定义:C”属性为Callendar-Van Dusen公式提供所需系数。 |
| Pt3750 | Pt3750。                                                     |
| Pt3851 | Pt3851。                                                     |
| Pt3911 | Pt3911。                                                     |
| Pt3916 | Pt3916。                                                     |
...
**r0**是0摄氏度时传感器的电阻值，以欧姆为单位。Callendar-Van Dusen方程需要该值。关于确定该值的详细信息见相关传感器文档。

**错误输出**包含错误信息。如**错误输入**表明该节点运行前已发生错误，则**错误输出**将包含相同的错误信息。否则，**错误输出**表示该节点产生的错误状态。

##### 4.1.1.23 热敏电阻电流激励

**创建通道，使用热敏电阻测量温度。如热敏电阻需电流激励，可使用该实例。**

![DAQmx Create Channel (AI-Temperature-Thermistor-Iex).gvi](Image/DAQmx Create Channel (AI-Temperature-Thermistor-Iex).gvi.png)
...
**热敏电阻特性**包含Steinhart-Hart热敏电阻公式中的常量。关于确定常量值的更多信息，见传感器相关文档。
